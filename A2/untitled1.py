# -*- coding: utf-8 -*-
"""
Created on Wed Mar 27 14:08:59 2024
...
y  = data[:, 4:5] # Get the body mass (target)

X = data[:,[1,2,3]] # Get the features (species, bill_length, bill_depth, flipper_length)
# # Normalize data
X = stats.zscore(X)
# X = np.concatenate((data[:,0:1],X),1)


# species = np.array(X[:, 0], dtype=int).T
...
# X = np.concatenate((X[:, :1], species_encoding, X[:, 1:]), axis=1)
# X[:, 0] = X[:, 1] # Make the first column equal the second column

# X = np.delete(X, 1, axis=1) # Delete the second column
